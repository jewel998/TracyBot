...
#---------------------
  def location(self):
    send_url = "http://ip-api.com/json/14.139.249.201"
    ...
    region = j["region"]
    country = j["country"]
    answer = "You are at latitude "+str(lat)+" degrees and longitude "+str(lon)+" degrees location is "+str(org)+str(city)+str(region)+str(country)
    return answer
...
def respond(text):
  ...
  if text == "open browser":
    Tracy.openbrowser(1)
  elif text == "Google this" or text == "search online":
    Tracy.search(1)
  elif re.match(r'(define+|search+|Google+)(.*)',text) is not None:
    if re.match(r'define(.*)',text):
      text = text.replace("define","",1)
    elif re.match(r'Google(.*)',text):
      text = text.replace("Google","",1)
    else:
      text = text.replace("search","",1)
    return google.search(text)
  elif text == "my IP":
    return Tracy.mypublicip()
  elif text == "my location":
    return Tracy.location()
  elif re.match(r'set(.*)reminder(.*)', text) is not None:
    return birthdayReminder.addBirthdays(text)
  elif re.match(r'check(.*)reminder(.*)', text) is not None:
    return birthdayReminder.checkTodaysBirthdays()
  return friend.respond(text)
